...
func (m *UserModel) Insert(name, email, password string) error {
	...
	if err != nil {
		if mySQLError, ok := errors.AsType[*mysql.MySQLError](err); ok {
			if mySQLError.Number == ErDupEntry && strings.Contains(mySQLError.Message, "users_uc_email") {
...
func (m *UserModel) PasswordUpdate(id int, currentPassword, newPassword string) error {
	var hashedCurrPassword []byte
	stmt := "SELECT hashed_password FROM users WHERE id = ?"

	err := m.DB.QueryRow(stmt, id).Scan(&hashedCurrPassword)
	if err != nil {
		return err
	}

	err = bcrypt.CompareHashAndPassword(hashedCurrPassword, []byte(currentPassword))
	if err != nil {
		if errors.Is(err, bcrypt.ErrMismatchedHashAndPassword) {
